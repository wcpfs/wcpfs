...
function fetch(url) {
  return $.getJSON(url).pipe(function(result) { return result; });
}
...
function userInfoPromise() {
  ...
  _userInfoPromise = $.getJSON('/user/info').pipe(function(result) { return result; });
...
function populateGameTemplate(game, elem) {
  function updateJoinButton(button) {
    if (game.seats.length < 6) {
      button.attr('href', '/user/joinGame?id=' + game.id);
    } else {
